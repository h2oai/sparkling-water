...
class H2OInterpreter(sparkContext: SparkContext, sessionId: Int) extends BaseH2OInterpreter(sparkContext, sessionId) {
  ...
  override def createSettings(): Settings = {
    ...
    val jars = getUserJars(conf, isShell = true).mkString(File.pathSeparator)
...
object H2OInterpreter {
  ...
  def classServerUri: String = throw new RuntimeException("classServerUri field is not defined when running in repl from spark 2.0")
}
