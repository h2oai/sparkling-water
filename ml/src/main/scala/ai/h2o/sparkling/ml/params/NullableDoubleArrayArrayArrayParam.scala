...
class NullableDoubleArrayArrayArrayParam(
    parent: Params,
    name: String,
    doc: String,
    isValid: Array[Array[Array[Double]]] => Boolean)
  extends Param[Array[Array[Array[Double]]]](parent, name, doc, isValid) {
  ...
  override def jsonEncode(value: Array[Array[Array[Double]]]): String = {
    if (value == null) {
      ...
      compact(render(value.toSeq.map(_.toSeq.map(_.toSeq.map(v => DoubleParam.jValueEncode(v))))))
    }
  }

  override def jsonDecode(json: String): Array[Array[Array[Double]]] = {
    parse(json) match {
      case JNull =>
        null
      ...
      case _ =>
        throw new IllegalArgumentException(s"Cannot decode $json to Array[Array[Array[Double]]].")
    }
  }
}
