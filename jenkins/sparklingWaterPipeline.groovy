        ...
        stage('QA: Unit Tests - ' + config.backendMode) {
            if (config.runUnitTests.toBoolean()) {
                try {
                    ...
                    testReport 'core/build/reports/tests/test', "Core Unit tests ${config.sparkMajorVersion} ${config.backendMode}"
                    junit 'ml/build/test-results/test/*.xml'
                    testReport 'ml/build/reports/tests/test', "ML Unit Tests ${config.sparkMajorVersion} ${config.backendMode}"
                    junit 'repl/build/test-results/test/*.xml'
                    testReport 'repl/build/reports/tests/test', "REPL Unit Tests ${config.sparkMajorVersion} ${config.backendMode}"
                    junit 'utils/build/test-results/test/*.xml'
                    testReport 'utils/build/reports/tests/test', "Utils Unit Tests ${config.sparkMajorVersion} ${config.backendMode}"
                    junit 'macros/build/test-results/test/*.xml'
                    testReport 'macros/build/reports/tests/test', "Macros Unit Tests ${config.sparkMajorVersion} ${config.backendMode}"
                }
            }
        }
    }
        ...
        stage('QA: Integration Tests - ' + config.backendMode) {
            if (config.runIntegTests.toBoolean()) {
                try {
                    ...
                    testReport 'core/build/reports/tests/integTest', "Core Integration tests ${config.sparkMajorVersion} ${config.backendMode}"
                    junit 'examples/build/test-results/integTest/*.xml'
                    testReport 'examples/build/reports/tests/integTest', "Examples Integration tests ${config.sparkMajorVersion} ${config.backendMode}"
                    junit 'ml/build/test-results/integTest/*.xml'
                    testReport 'ml/build/reports/tests/integTest', "ML Integration tests ${config.sparkMajorVersion} ${config.backendMode}"
                }
            }
        }
    }
