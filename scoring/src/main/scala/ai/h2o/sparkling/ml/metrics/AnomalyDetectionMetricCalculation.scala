...
import hex.tree.isofor.ModelMetricsAnomaly.IndependentMetricBuilderAnomaly

trait AnomalyDetectionMetricCalculation {
  self: H2OUnsupervisedMOJOModel =>

  override private[sparkling] def makeMetricBuilder(wrapper: EasyPredictModelWrapper): IndependentMetricBuilder[_] = {
    new IndependentMetricBuilderAnomaly("Anomaly metrics", false)
  }
