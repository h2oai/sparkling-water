...
trait H2OMOJOPredictionUpliftBinomial {
  ...
  def getUpliftBinomialPredictionUDF(
      schema: StructType,
      modelUID: String,
      mojoFileName: String,
    ...
    val function = (r: Row) => {
      ...
      val pred = model.predictUpliftBinomial(RowConverter.toH2ORowData(r))
      val resultBuilder = mutable.ArrayBuffer[Any]()
      resultBuilder += pred.predictions
      new GenericRowWithSchema(resultBuilder.toArray, schema)
    }
    udf(function, schema)
  }

  def getUpliftBinomialPredictionColSchema(): Seq[StructField] = {
    Seq(StructField(getPredictionCol(), predictionColType, nullable = predictionColNullable))
  }

  def getUpliftBinomialPredictionSchema(): StructType = {
    val valueField = StructField("value", predictionColType, nullable = false)

    StructType(valueField :: Nil)
  }

  def extractUpliftBinomialPredictionColContent(): Column = {
