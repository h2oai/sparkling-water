...
object H2OMOJOModel extends H2OMOJOReadable[H2OMOJOModel] with H2OMOJOLoader[H2OMOJOModel] with H2OMOJOModelUtils {
  ...
  // When H2OMOJOModel is created from existing mojo created in H2O-3, we do not need to set original features, but as in Spark, data frames can be nested, we need to handle it
  private[h2o] def createFromMojo(mojoData: Array[Byte], uid: String, settings: H2OMOJOSettings, originalFeatures: Array[String]): H2OMOJOModel = {
