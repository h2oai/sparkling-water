...
class H2OTargetEncoderMOJOModel(override val uid: String)
  ...
  @transient private lazy val orderOfInputColumns = {
    val mojoModel = Utils.getMojoModel(getMojoLocalPath()).asInstanceOf[TargetEncoderMojoModel]
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val udfWrapper = H2OTargetEncoderMOJOUdfWrapper(getMojoLocalPath(), outputCols)
...
/**
  ...
  */
case class H2OTargetEncoderMOJOUdfWrapper(mojoPath: String, outputCols: Array[String]) {
  @transient private lazy val easyPredictModelWrapper: EasyPredictModelWrapper = {
    val model = Utils.getMojoModel(mojoPath).asInstanceOf[TargetEncoderMojoModel]
