...
class H2OContextSSLTestSuite extends FunSuite with SharedH2OTestContext {
  ...
  test("H2O endpoint behind Flow Proxy is accessible with HTTPS") {
    val cloudV3 =
      RestApiUtils.query[CloudV3](new URI(s"https://${getFlowIp()}:${getFlowPort()}"), "3/Cloud", hc.getConf)
    assert(cloudV3 != null)
  ...
  test("SW endpoint behind Flow Proxy is accessible with HTTPS") {
    ...
    val dataFrames =
      RestApiUtils.query[DataFrames](new URI(s"https://${getFlowIp()}:${getFlowPort()}"), "3/dataframes", hc.getConf)
