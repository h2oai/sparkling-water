...
object SharedBackendConf {
  ...
  /** Enable or disable web on h2o worker nodes. It is disabled by default for security reasons except for case
    * we are using rest api based client solution where this option has no meaning as we need to have
    * web enabled on the worker nodes. */
