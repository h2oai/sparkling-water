...
object SupportedRDDConverter {
  ...
  def toRDD[A <: Product : TypeTag : ClassTag, T <: Frame](hc: H2OContext, fr: T): H2ORDD[A, H2OFrame] = new H2ORDD[A, H2OFrame](new H2OFrame(fr))(hc)
}
...
private[this] object SupportedRDD {
  ...
  implicit def toH2OFrameFromRDDProductNoTypeTag(rdd: RDD[Product]): SupportedRDD = new SupportedRDD {
    override def toH2OFrame(hc: H2OContext, frameKeyName: Option[String]): H2OFrame = {
      SparkDataFrameConverter.toH2OFrame(hc, hc.sparkSession.createDataFrame(rdd), frameKeyName)
    }
  }
