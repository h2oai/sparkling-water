  ...
  implicit def toH2OFrameFromDatasetJavaBool(dataset: Dataset[java.lang.Boolean]): SupportedDataset =
    new SupportedDataset {
      override def toH2OFrameKeyString(hc: H2OContext, frameKeyName: Option[String]): String = {
        SparkDataFrameConverter.toH2OFrameKeyString(hc, dataset.toDF(), frameKeyName)
      }
    }
  ...
  implicit def toH2OFrameFromDatasetTimeStamp(dataset: Dataset[java.sql.Timestamp]): SupportedDataset =
    new SupportedDataset {
      override def toH2OFrameKeyString(hc: H2OContext, frameKeyName: Option[String]): String = {
        ...
        SparkDataFrameConverter.toH2OFrameKeyString(hc, dataset.map(v => Tuple1(v)).toDF(), frameKeyName)
      }
    }
