...
class RDDsHandler(val sc: SparkContext, val h2oContext: H2OContext) extends Handler {
  ...
  private[RDDsHandler] def convertToH2OFrame(rdd: RDD[_], name: Option[String]): H2OFrame = {
    if (rdd.isEmpty()) {
      ...
      H2OFrame(h2oContext.asH2OFrameKeyString(sc.parallelize(Seq.empty[Int]), name))
    } else {
      rdd.first() match {
        case _: Double => H2OFrame(h2oContext.asH2OFrameKeyString(rdd.asInstanceOf[RDD[Double]], name))
        case _: LabeledPoint => H2OFrame(h2oContext.asH2OFrameKeyString(rdd.asInstanceOf[RDD[LabeledPoint]], name))
        case _: Boolean => H2OFrame(h2oContext.asH2OFrameKeyString(rdd.asInstanceOf[RDD[Boolean]], name))
        case _: String => H2OFrame(h2oContext.asH2OFrameKeyString(rdd.asInstanceOf[RDD[String]], name))
        ...
        case _: Product =>
          ...
          H2OFrame(h2oContext.asH2OFrameKeyString(df, name))
