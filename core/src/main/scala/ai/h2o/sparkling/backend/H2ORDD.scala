...
private[backend] class H2ORDD[A <: Product: TypeTag: ClassTag](val frame: H2OFrame, val productType: ProductType)(
  ...
  def this(@transient frame: H2OFrame)(@transient hc: H2OContext) = this(frame, ProductType.create[A])(hc)
    ...
    if (!productType.isSingleton) {
      ...
      if (problems.nonEmpty) {
        throw new IllegalArgumentException(
          s"The following fields are missing in frame: $problems; " +
  ...
  private def opt[X](op: => Any): Option[X] =
    try {
  ...
  abstract class H2ORDDIterator extends H2OChunkIterator[A] {
    ...
    private def readOneRow(): Option[A] = {
      ...
      res.toList match {
        ...
        case one :: two :: more =>
          throw new scala.IllegalArgumentException(s"found more than one $jc constructor for given args - can't choose")
      }
    }
  }
  ...
  lazy val constructors: Seq[Constructor[_]] = {
    ...
    if (found.isEmpty)
      throw new scala.IllegalArgumentException(s"Constructor must take exactly ${productType.arity} args")
  ...
  @(transient @field @getter)
  private lazy val instanceBuilders = constructors map InstanceBuilder
}
