...
class ExternalH2OBackend(val hc: H2OContext) extends SparklingBackend with ShellUtils {
  ...
  private def launchExternalH2OOnYarn(conf: H2OConf): Unit = {
    ...
    val proc = launchShellCommand(cmdToLaunch)
  ...
  override def checkAndUpdateConf(conf: H2OConf): H2OConf = {
    ...
    if (conf.isAutoClusterStartUsed) {
      ...
      if (conf.getOption("spark.ext.h2o.external.kerberos.principal").isDefined && conf.kerberosPrincipal.isEmpty) {
        logWarning(
          s"The option 'spark.ext.h2o.external.kerberos.principal' is deprecated and will be removed in version 3.32." +
      ...
      if (conf.getOption("spark.ext.h2o.external.kerberos.keytab").isDefined && conf.kerberosKeytab.isEmpty) {
        logWarning(
          s"The option 'spark.ext.h2o.external.kerberos.keytab' is deprecated and will be removed in version 3.32." +
      ...
      if (conf.getOption("spark.yarn.principal").isDefined &&
        ...
        logInfo(
          s"spark.yarn.principal provided and ${SharedBackendConf.PROP_KERBEROS_PRINCIPAL._1} is" +
      ...
      if (conf.getOption("spark.yarn.keytab").isDefined &&
        ...
        logInfo(
          s"spark.yarn.keytab provided and ${SharedBackendConf.PROP_KERBEROS_KEYTAB._1} is" +
            s" not set. Passing the configuration to H2O.")
      ...
      if (conf.isHiveSupportEnabled && conf.runAsUser.isEmpty) {
        ...
        logInfo(
          s"Setting property ${ExternalBackendConf.PROP_EXTERNAL_RUN_AS_USER._1} to the spark user '$sparkUser'" +
