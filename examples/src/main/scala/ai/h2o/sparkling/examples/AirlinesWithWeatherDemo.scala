...
object AirlinesWithWeatherDemo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val airlinesTable = spark.read.option("inferSchema", "true").option("header", "true").csv(airlinesDataFile)

    val flightsToORD = airlinesTable.filter('Dest === "ORD")

    println(s"\nFlights to ORD: ${flightsToORD.count}\n")

    flightsToORD.createOrReplaceTempView("FlightsToORD")
    weatherTable.createOrReplaceTempView("WeatherORD")
    ...
    val predictionsFromDL = deepLearningModel.transform(joined).collect()
    ...
    val predictionsFromGBM = gbmModel.transform(joined).collect()
