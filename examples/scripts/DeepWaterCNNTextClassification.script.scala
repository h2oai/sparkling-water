...
def vectorize(sentences: RDD[(Int, Array[String])], vocabSize: Int) = {
  sentences.map { case (l, s) => (l, {
    val htf = new HashingTF(vocabSize)
    s.map{str =>htf.indexOf(str)}
  })}
}
...
p._image_shape = Array(56, 1)
p._channels = 1
p._problem_type == DeepWaterParameters.ProblemType.dataset
