...
## Script
The script is available at GitHub - [https://raw.githubusercontent.com/h2oai/sparkling-water/master/examples/scripts/Meetup20150422.script.scala](https://raw.githubusercontent.com/h2oai/sparkling-water/master/examples/scripts/Meetup20150422.script.scala).
...
> Note: You can directly execute script:
```bash
...
*Census* dataset is available in [GitHub](https://raw.githubusercontent.com/h2oai/sparkling-water/master/examples/smalldata/chicagoAllWeather.csv).
...
14. Create an application scoring method
   ```scala
...
17. Create Spark's SchemaRDD with `crimeType`, `numberOfCrimes`, `numberOfArrest`,...
   ```scala
   ...
   ```
18. Publish the created SchemaRDD as H2O's Frame:
   ```scala
   val rateFrame:DataFrame = rateSRdd
   ```
   
19. Visualize number of crimes in the H2O UI Flow. Type given commands into Flow's cell:
   ```coffe
plot (g) -> g(
  g.rect(
