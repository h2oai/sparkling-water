...
class HadoopSmokeTestSuite(unittest.TestCase):
        ...
        def test_import_orc_hdfs(self):
                userdata_orc_hdfs = "hdfs://172.17.0.24/user/h2o/userdata1_orc"
                userdata_orc_hdfs_df = h2o.import_file(path=userdata_orc_hdfs,header=1)
                assert userdata_orc_hdfs_df.ncol == 13
                assert userdata_orc_hdfs_df.nrow == 1000
                assert userdata_orc_hdfs_df[0, 2] == "Amanda"
                assert userdata_orc_hdfs_df[4, 4] == "cburns4@miitbeian.gov.cn"
                assert userdata_orc_hdfs_df[999, 7] == "374288099198540"

        def test_export_orc_hdfs(self):
                userdata_orc_hdfs = "hdfs://172.17.0.24/user/h2o/userdata1_orc"
                userdata_orc_hdfs_df = h2o.import_file(path=userdata_orc_hdfs,header=1)
                userdata_orc_hdfs_export = "hdfs://172.17.0.24/user/h2o/userdata1_export_orc"
                isOK = True
                try:
